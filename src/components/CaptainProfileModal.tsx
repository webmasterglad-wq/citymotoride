            ...
            {activeTab === 'profile' && (
              <div className="space-y-4">
                ...
                {!isEditing ? (
                    ...
                    <div className={`border rounded-2xl p-4 space-y-3 ${
                      ...
                      <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-xs font-black uppercase tracking-wider ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                        <div className={`p-2.5 rounded-xl border ${isLight ? 'bg-white border-slate-200' : 'bg-slate-950/60 border-slate-800'}`}>
                          <span className={`text-[10px] block uppercase font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Full Name</span>
                          <span className={`text-xs font-black block mt-0.5 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>{captain.name}</span>
                        </div>
                        <div className={`p-2.5 rounded-xl border ${isLight ? 'bg-white border-slate-200' : 'bg-slate-950/60 border-slate-800'}`}>
                          <span className={`text-[10px] block uppercase font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Phone Number</span>
                          <span className={`text-xs font-black block mt-0.5 ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>{captain.phone}</span>
                        </div>
                        <div className={`p-2.5 rounded-xl border sm:col-span-2 ${isLight ? 'bg-white border-slate-200' : 'bg-slate-950/60 border-slate-800'}`}>
                          <span className={`text-[10px] block uppercase font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Driver Email ID</span>
                          <span className={`text-xs font-black block mt-0.5 truncate ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>{email}</span>
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className={`w-full py-2 font-bold rounded-xl border transition-colors cursor-pointer text-xs ${
                          isLight
                  ...
                  <form onSubmit={handleSaveProfile} className="space-y-3 p-4 rounded-2xl border bg-amber-500/5 border-amber-500/20">
                    <div className="flex items-center justify-between pb-1 border-b border-amber-500/20">
                      ...
                      <span className="text-[10px] text-slate-400">All fields mandatory</span>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <button
                        type="submit"
                        className="flex-1 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-xl shadow-md cursor-pointer"
                      >
                        Save Personal Info
                      </button>
                    </div>
                ...
                {/* Whole Registered Bike Details & Live Photo Card */}
                <div className={`border rounded-2xl p-4 space-y-3.5 transition-all ${
                  isLight ? 'bg-slate-50 border-slate-200 text-slate-800' : 'bg-slate-900/80 border-slate-800 text-slate-200'
                }`}>
                  {/* Bike Section Header with Status & Action */}
                  <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className={`text-xs font-black uppercase tracking-wider ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                          Registered Bike & Vehicle Details
                        </h4>
                        <span className="text-[10px] text-slate-400">Certified Commercial Two-Wheeler</span>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={handleOpenBikeEditor}
                      className="px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 transition-all cursor-pointer shadow-xs flex items-center gap-1"
                    >
                      {isEditingBike ? 'Close Editor' : (vehicleDetails ? 'Edit Bike' : 'Register Bike')}
                    </button>
                  </div>

                  {/* Bike Image & Specifications Grid */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
                    {/* Bike Photo Showcase */}
                    <div className="relative group/bike overflow-hidden rounded-xl border border-amber-500/30 bg-slate-950 flex flex-col items-center justify-center p-2 min-h-[110px]">
                      {bikeImage ? (
                        <>
                          <img
                            src={bikeImage}
                          ...
                          />
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/bike:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <button
                              type="button"
                              onClick={() => bikeImageInputRef.current?.click()}
                              className="p-1.5 rounded-full bg-amber-500 text-slate-950 shadow-md cursor-pointer hover:scale-110 transition-transform"
                              title="Change Bike Photo"
                            >
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-full bg-rose-600 text-white shadow-md cursor-pointer hover:scale-110 transition-transform"
                              title="Remove Photo"
                            >
                        ...
                        <div className="text-center p-2">
                          <Bike className="w-8 h-8 text-amber-500/50 mx-auto mb-1" />
                          <button
                            type="button"
                            onClick={() => bikeImageInputRef.current?.click()}
                            className="text-[10px] font-bold text-amber-500 hover:underline flex items-center justify-center gap-1 cursor-pointer mx-auto"
                          >
                            <Camera className="w-3 h-3" /> Add Bike Photo
                          </button>
                        </div>
                      )}
                    </div>

                    {/* Bike Specifications Matrix */}
                    <div className="sm:col-span-2 space-y-2">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className={`p-2 rounded-xl border ${isLight ? 'bg-white border-slate-200' : 'bg-slate-950/60 border-slate-800'}`}>
                          <span className={`text-[9px] block uppercase font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Bike Model</span>
                          <span className={`font-black text-xs block truncate ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                            {bikeModel || (vehicleDetails ? vehicleDetails.split('·')[0].split('#')[0].trim() : 'Hero Splendor+ / Activa')}
                          </span>
                        </div>
                        <div className={`p-2 rounded-xl border ${isLight ? 'bg-white border-slate-200' : 'bg-slate-950/60 border-slate-800'}`}>
                          <span className={`text-[9px] block uppercase font-bold ${isLight ? 'text-slate-400' : 'text-slate-500'}`}>Color</span>
                          <span className={`font-black text-xs block truncate ${isLight ? 'text-slate-900' : 'text-slate-100'}`}>
                            {bikeColor || 'Standard Fleet Color'}
                          </span>
                        </div>
                      </div>

                      {/* High Security Registration Plate (HSRP) Badge */}
                      <div className="flex items-center justify-between p-2 rounded-xl bg-gradient-to-r from-amber-500/15 via-slate-900/50 to-slate-900 border border-amber-500/30">
                        <div>
                          <span className="text-[9px] uppercase font-bold text-amber-500 block">Registration Plate (HSRP)</span>
                          <span className="font-mono font-black text-sm tracking-wider text-slate-100">
                            {licensePlate || bikePlateInput || 'CH01-AB-1234'}
                          </span>
                        </div>
                  ...
                  {/* Service Eligibility & Verified Document Pills */}
                  <div className="space-y-2 pt-1">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="text-[10px] font-bold text-slate-400">Services:</span>
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-500/30 flex items-center gap-1">
                        🛵 Moto Comfort Ride
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-cyan-500/15 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30 flex items-center gap-1">
                        📦 Moto Courier
                      </span>
                    </div>

                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 text-center text-[10px]">
                      <div className={`p-1.5 rounded-lg border flex items-center justify-center gap-1 font-bold ${
                        isLight ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                      }`}>
                        <FileCheck className="w-3 h-3" /> RC Verified
                      </div>
                      <div className={`p-1.5 rounded-lg border flex items-center justify-center gap-1 font-bold ${
                        isLight ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                      }`}>
                        <ShieldCheck className="w-3 h-3" /> Insurance Active
                      </div>
                      <div className={`p-1.5 rounded-lg border flex items-center justify-center gap-1 font-bold ${
                        isLight ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                      }`}>
                        <CheckCircle2 className="w-3 h-3" /> PUC Passed
                      </div>
                      <div className={`p-1.5 rounded-lg border flex items-center justify-center gap-1 font-bold ${
                        isLight ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                      }`}>
                        <Shield className="w-3 h-3" /> Helmet Ready
                      </div>
                    </div>
                  </div>

                  {/* Inline Bike Editor (Expandable inside main profile view) */}
                  {isEditingBike && (
                    <form onSubmit={handleSaveBike} className="mt-3 pt-3 border-t border-dashed border-amber-500/30 space-y-3 animate-in fade-in duration-150">
                      <div className="text-xs font-bold text-amber-500 uppercase tracking-wider">
                        Update Registered Bike & Plate Details
                      </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 space-y-1">
                            ...
                            <p className="text-[10px] text-slate-400">
                              Upload a photo of your registered bike.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        >
                          <CheckCircle className="w-3.5 h-3.5" /> Save Bike Details
                        </button>
                      </div>
                    </form>
                  )}
